import pandas as pd
from FinModel.BSM_Model import BSM_Model
...
def main():
    data = pd.read_csv("S50ETF.csv")  # 读取原始数据
    ...
    error_df = pd.DataFrame(
        {
            ...
            "FinModel Error": heston_error_mean.values,
        }
    )
    ...
    plt.plot(error_df["TT"], error_df["FinModel Error"], label="Heston模型平均误差率")
    ...
    plt.xlabel("交易日期")
    plt.ylabel("平均误差率")
    plt.title("BSM模型与Heston模型的平均误差率随时间变化图_")
    plt.legend()  # 显示图例
    plt.grid(True)  # 显示网格
    ...
    plt.xlabel("模型")
    plt.ylabel("平均误差率")
    plt.title("BSM模型与Heston模型的总平均误差率比较_")
    plt.ylim(
